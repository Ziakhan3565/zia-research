...
st.markdown("""
<style>
:root{--bg:#05070b;--panel:#0b1119;--panel2:#101925;--line:#1c2a39;--txt:#edf3fb;--muted:#7d8ba0;--green:#42dda0;--red:#ff7184;--amber:#f3c86a;--cyan:#65d7ff;--violet:#969eff}
html,body,[data-testid="stAppViewContainer"]{background:var(--bg);color:var(--txt)}
[data-testid="stHeader"]{background:transparent}.block-container{max-width:1920px;padding:12px clamp(8px,1.7vw,34px) 35px}
.hero{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--line);padding:2px 2px 13px;margin-bottom:10px}.brand{font-size:clamp(24px,2.8vw,40px);font-weight:950;letter-spacing:-1.8px}.brand b{color:var(--violet)}.micro{color:var(--muted);font-size:9px;letter-spacing:1.4px}.live{border:1px solid #245d45;background:#071810;color:#6ce3a5;border-radius:999px;padding:7px 12px;font-size:10px;font-weight:900}.dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 12px var(--green);margin-right:6px}
.panel{background:linear-gradient(145deg,#0d141e,#080d14);border:1px solid var(--line);border-radius:15px;padding:13px;margin-bottom:10px}.card{background:linear-gradient(145deg,#111a26,#0a1018);border:1px solid var(--line);border-radius:14px;padding:12px;min-height:78px}.label{font-size:9px;color:var(--muted);font-weight:900;letter-spacing:1.1px}.value{font-size:20px;font-weight:950;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sub{font-size:10px;color:#8794a8;margin-top:3px}.section-title{font-size:18px;font-weight:950;margin:5px 0 2px}.section-sub{font-size:10px;color:var(--muted);margin-bottom:9px}.good{color:var(--green)}.bad{color:var(--red)}.amber{color:var(--amber)}.violet{color:var(--violet)}.cyan{color:var(--cyan)}
.signalbox{border-radius:18px;padding:18px 20px;border:1px solid var(--line);background:linear-gradient(145deg,#111a26,#080d14);text-align:center}.signal-long{border-color:#277b59;box-shadow:0 0 28px rgba(66,221,160,.08)}.signal-short{border-color:#843a4a;box-shadow:0 0 28px rgba(255,113,132,.08)}.signal-wait{border-color:#705e30}.signal-main{font-size:clamp(34px,4vw,58px);font-weight:1000;letter-spacing:-2px;line-height:1}.signal-meta{font-size:10px;color:var(--muted);margin-top:7px;letter-spacing:1px}
...
@media(max-width:700px){.block-container{padding:6px 7px 25px}.brand{font-size:21px}.micro{font-size:7px}.live{font-size:8px;padding:5px 8px}.panel{padding:9px;border-radius:12px}.card{min-height:62px;padding:9px}.value{font-size:16px}.section-title{font-size:14px}}
</style>
...
def visible_tri_timeframes(tf):
    if tf in ("1H","4H"): return [("1W","WEEKLY"),("1D","DAILY")]
    if tf in ("5M","15M","30M"): return [("1H","1 HOUR"),("4H","4 HOUR")]
    return []
...
def make_chart(df,symbol,tf,future):
    ...
    colors={"1W":"#8e98ff","1D":"#65d7ff","1H":"#42dda0","4H":"#f3c86a"}
    for label,_ in visible_tri_timeframes(tf):
        lv=tri_levels(symbol,TFS[label])
        if not lv:continue
        col=colors[label]
        fig.add_hline(y=lv["body"],line_color=col,line_width=1.8,annotation_text=f"TRI {label} • BODY 50",annotation_position="top right")
        fig.add_hline(y=lv["upper"],line_color=col,line_width=1.0,line_dash="dot",annotation_text=f"{label} • UPPER 50",annotation_position="top left")
        fig.add_hline(y=lv["lower"],line_color=col,line_width=1.0,line_dash="dot",annotation_text=f"{label} • LOWER 50",annotation_position="bottom left")
    step=view.Time.iloc[-1]-view.Time.iloc[-2] if len(view)>1 else pd.Timedelta(minutes=5)
    fig.update_xaxes(range=[view.Time.iloc[0],view.Time.iloc[-1]+step*future],rangeslider_visible=False,showgrid=True,gridcolor="#172230",showspikes=True,spikemode="across")
    fig.update_yaxes(side="right",showgrid=True,gridcolor="#172230",fixedrange=False)
    fig.update_layout(height=650,margin=dict(l=4,r=4,t=8,b=8),paper_bgcolor="#080d14",plot_bgcolor="#080d14",font=dict(color="#cbd5e1"),hovermode="x unified",dragmode="pan",uirevision=f"{symbol}-{tf}",legend=dict(orientation="h",y=1.02,x=0))
    return fig
...
if visible:
    names=" + ".join(x[0] for x in visible)
    st.caption(f"AUTO TRI • {tf} chart automatically shows {names} TRI lines • controls removed")
else:
    st.caption(f"AUTO TRI • no manual TRI controls on {tf} • chart remains clean")
...
def live_engine():
    ...
    with tabs[1]:
        st.markdown(f'<div class="panel"><b>TRADINGVIEW-STYLE MARKET CHART</b><div class="section-sub">Zoom • pan • crosshair • future space • automatic TRI: {", ".join(x[0] for x in visible) if visible else "none for this timeframe"}</div>',unsafe_allow_html=True)
        st.plotly_chart(make_chart(df,symbol,tf,future),use_container_width=True,config={"scrollZoom":True,"displaylogo":False,"responsive":True,"doubleClick":"reset"},key="main_market_chart")
