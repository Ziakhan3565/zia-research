...
def _append_feedback(trade):
    ...
    if str(trade.get("outcome", "")).upper() not in (
        "WIN",
        "LOSS",
    ):
        return
...
def _retrain_xgb_from_feedback(current_model):
    ...
    if (
        len(fb) < MIN_FEEDBACK_TO_RETRAIN
        or fb["target"].nunique() < 2
    ):
    ...
    if (
        len(fb) < MIN_FEEDBACK_TO_RETRAIN
        or len(fb) < last_count + RETRAIN_EVERY
    ):
        return current_model, None
    ...
    if test_acc < MIN_TEST_ACCURACY:
        ...
        return (
            current_model,
            f"XGB retrain rejected: holdout accuracy "
            f"{test_acc * 100:.1f}% < "
            f"{MIN_TEST_ACCURACY * 100:.0f}%",
        )
    ...
    return (
        candidate,
        f"XGB retrained from {len(fb)} completed trades | "
        f"holdout accuracy {test_acc * 100:.1f}%",
    )
...
def normalize_trade(trade):
    ...
    if not trade.get("entry_candle_time"):
        trade["entry_candle_time"] = trade.get(
            "timestamp",
            "",
        )
    ...
    if "signal_strength" not in trade:
        if trade.get("direction") == "LONG":
            trade["signal_strength"] = "BUY"
        elif trade.get("direction") == "SHORT":
            trade["signal_strength"] = "SELL"
        else:
            trade["signal_strength"] = "WAIT"
...
def resolve_pending_trades(
    ...
    current_candle_str = pd.Timestamp(
        current_candle_time
    ).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    for trade in history:
        ...
        tp_hit = (
            float(candle_high) >= tp
            if direction == "LONG"
            else float(candle_low) <= tp
        )

        sl_hit = (
            float(candle_low) <= sl
            if direction == "LONG"
            else float(candle_high) >= sl
        )
...
def fetch_klines_data(
    ...
    url = (
        ...
        f"&interval={binance_tf}&limit={limit}"
    )
...
if (
    ...
    long_votes = agreement_votes.count(
        "LONG"
    )

    short_votes = agreement_votes.count(
        "SHORT"
    )
    ...
    signal_color = (
        ...
        if signal_strength == "STRONG BUY"
            ...
            "#ff5252"
            if signal_strength == "STRONG SELL"
            else dir_color
        )
    )
...
• <b>HAWKES:</b>
Measures aggressive order clustering and arrival rates.

<br>

• <b>BOOK_IMB:</b>
Computes real-time depth pressure across bids & asks.

<br>

• <b>Dynamic Weights:</b>
Optimized linear blending for robust signals.
...
""",
    ...
    st.subheader(
        "📊 Performance Summary & Win Rate Checker with Filters"
    )
    ...
    if st.session_state.trade_history_log:
        ...
        ) if not losing_trades_df.empty else 0.0
