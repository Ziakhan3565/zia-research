...
# ==========================================
# 1. STREAMLIT CONFIG & PERSISTENT CSV SETUP
# ==========================================
st.set_page_config(
...
# Har 5 seconds / 5000ms baad page rerun hoga
...
class TRILineEngine:
    ...
    def get_tri_levels(self):
        levels_data = {}
        for tf_name, interval in self.enabled.items():
            try:
                url = f"https://api.binance.com/api/v3/klines?symbol={self.symbol}&interval={interval}&limit=1"
                ...
                if isinstance(res, list) and len(res) > 0:
                    ...
                    levels_data[tf_name] = TRILineLevels(
                        timeframe=tf_name,
                        open=o, high=h, low=l, close=c,
                        body_high=body_high,
                        body_low=body_low,
                        body_50=(body_high + body_low) / 2,
                        upper_50=(h + body_high) / 2,
                        lower_50=(l + body_low) / 2
                    )
            except Exception:
                continue
        return levels_data
...
# ==========================================
# 3. RESEARCH LAB & RISK ENGINE MODULES
# ==========================================
...
# ==========================================
# 5. SIDEBAR CONTROLS & NEW TIMEFRAMES
# ==========================================
...
TIMEFRAME_MAP = {
    ...
    "4h (Intraday)": ("4h", 240),
    "1d (Daily TF)": ("1d", 1440),
    "1w (Weekly TF)": ("1w", 10080),
    "1M (Monthly TF)": ("1M", 43200),
}
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 🎛️ TRI Line Chart Engine")
enable_tri_lines = st.sidebar.toggle("Show TRI Engine Levels (Monthly, Weekly, Daily...)", value=True)

st.sidebar.markdown("---")
st.sidebar.markdown("### 🎛️ Paper Trading Mode")
paper_trading_mode = st.sidebar.toggle("Enable Live Paper Trading", value=True)

binance_interval, tf_minutes = TIMEFRAME_MAP[selected_tf_label]

# ==========================================
# 6. DATA FETCHING (SAFE API WITH FALLBACK)
# ==========================================
@st.cache_data(ttl=15)
def fetch_klines_data(symbol, interval, limit=100):
    ...
    try:
        ...
        dates = pd.date_range(end=datetime.datetime.now(), periods=limit, freq=interval.replace('m','min').replace('M','ME'))
        base_p = 60000.0
...
df = fetch_klines_data(selected_symbol, binance_interval)
bids, asks = fetch_order_book_depth(selected_symbol)

# ==========================================
# 7. ENGINE EXECUTION & SIGNAL GENERATION
# ==========================================
if not df.empty and len(df) >= 3 and len(bids) > 0 and len(asks) > 0:
    ...
    # ==========================================
    # 10. CHART, MICROSTRUCTURE & TRI ENGINE LINES
    # ==========================================
    col_chart, col_risk_panel = st.columns([2.5, 1])
    with col_chart:
        st.subheader(f"Price Trajectory & TRI Levels ({selected_symbol})")
        ...
        if enable_tri_lines:
            ...
            tri_levels = tri_engine.get_tri_levels()
    ...
    # ==========================================
    # 12. PERFORMANCE & WIN RATE SECTION
    # ==========================================
    st.markdown("---")
