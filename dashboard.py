...
TFS = {"1M":"1M", "1W":"1w", "1D":"1d", "4H":"4h", "1H":"1h", "30M":"30m", "15M":"15m", "5M":"5m", "3M":"3m", "1M1":"1m"}
TRI_TFS = ["1M", "1W", "1D", "4H", "1H", "30M", "15M", "5M"]

st.markdown("""
<style>
:root{--bg:#05070b;--panel:#0b1119;--panel2:#101925;--line:#1d2a39;--txt:#edf3fb;--muted:#7d8ba0;--violet:#8e98ff;--green:#42dda0;--red:#ff7184;--amber:#f3c86a;--cyan:#65d7ff}
html,body,[data-testid="stAppViewContainer"]{background:var(--bg);color:var(--txt)}
[data-testid="stHeader"]{background:rgba(5,7,11,.88)}
.block-container{max-width:1920px;padding:10px clamp(8px,1.7vw,34px) 40px}
.hero{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--line);padding:4px 2px 12px;margin-bottom:10px}.brand{font-size:clamp(23px,2.7vw,38px);font-weight:950;letter-spacing:-1.7px}.brand b{color:var(--violet)}.micro{color:var(--muted);font-size:9px;letter-spacing:1.5px}.live{border:1px solid #235c43;background:#071810;color:#6ce3a5;border-radius:999px;padding:7px 12px;font-size:10px;font-weight:900}.dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 12px var(--green);margin-right:6px}
.panel{background:linear-gradient(145deg,#0d141e,#080d14);border:1px solid var(--line);border-radius:15px;padding:13px;margin-bottom:10px}.card{background:linear-gradient(145deg,#111a26,#0a1018);border:1px solid var(--line);border-radius:14px;padding:12px;min-height:82px}.label{font-size:9px;color:var(--muted);font-weight:900;letter-spacing:1.1px}.value{font-size:21px;font-weight:950;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sub{font-size:10px;color:#8794a8;margin-top:3px}.section-title{font-size:18px;font-weight:950;margin:12px 0 2px}.section-sub{font-size:10px;color:var(--muted);margin-bottom:9px}.good{color:var(--green)}.bad{color:var(--red)}.amber{color:var(--amber)}.violet{color:var(--violet)}.cyan{color:var(--cyan)}
.signalbox{border-radius:18px;padding:20px 22px;border:1px solid var(--line);background:linear-gradient(145deg,#111a26,#080d14);text-align:center}.signal-long{border-color:#277b59;box-shadow:0 0 28px rgba(66,221,160,.08)}.signal-short{border-color:#843a4a;box-shadow:0 0 28px rgba(255,113,132,.08)}.signal-wait{border-color:#705e30}.signal-main{font-size:clamp(34px,4vw,58px);font-weight:1000;letter-spacing:-2px;line-height:1}.signal-meta{font-size:10px;color:var(--muted);margin-top:7px;letter-spacing:1px}
...
@media(max-width:700px){.block-container{padding:6px 7px 30px}.brand{font-size:21px}.micro{font-size:7px}.live{font-size:8px;padding:5px 8px}.panel{padding:9px;border-radius:12px}.card{min-height:65px;padding:9px}.value{font-size:16px}.section-title{font-size:14px}.stTabs [data-baseweb="tab"]{font-size:9px;padding:6px}}
</style>
...
def obi(bids, asks, k):
    ...
    bv, av = float(bids[:k,1].sum()), float(asks[:k,1].sum())
    return ((bv-av)/(bv+av) if bv+av else 0.0), bv, av


def features(df, b, a):
    f = {k:0.0 for k in ["top20_bid_sum","top20_ask_sum","obi_5","obi_10","obi_20","obi_50","spread","spread_pct","bid_ask_ratio_20","bid_ask_ratio_50","top20_total_depth","top50_total_depth","taker_buy_volume","taker_sell_volume","taker_flow","taker_flow_ratio","price_return","price_change","sma_distance","realized_volatility","BOOK_IMB","QUANT_IMPLY","ADAPT_CONF","BAYESIAN","FOURIER_TREND"]}
...
def ml_predict(f):
    ...
    try:
        names=list(model.get_booster().feature_names or []) if hasattr(model,"get_booster") else []
...
def final_state(f,p,pr):
    ...
    signal="LONG" if combined>=.45 else "SHORT" if combined<=-.45 else "WAIT"
...
def tri_levels(symbol, interval):
    ...
    if len(df)<2:
        return None
    # Use the last CLOSED candle, never the currently forming candle.
    c=df.iloc[-2]
    o,h,l,cl=map(num,[c.Open,c.High,c.Low,c.Close])
    body_hi,body_lo=max(o,cl),min(o,cl)
    return {"body":(body_hi+body_lo)/2,"upper":(h+body_hi)/2,"lower":(l+body_lo)/2,"time":c.Time}
...
def make_chart(df,symbol,tf,future,tri_settings):
    ...
    for span in (10,20,50,200):
        if len(view)>=span:
            fig.add_trace(go.Scatter(x=view.Time,y=view.Close.ewm(span=span,adjust=False).mean(),mode="lines",name=f"EMA {span}",line={"width":1.1}))
    for label,setting in tri_settings.items():
        if not setting.get("enabled"):
            continue
        levels=tri_levels(symbol,TFS[label])
        if not levels:
            continue
        colors=setting["colors"]
        fig.add_hline(y=levels["body"],line_color=colors[0],line_width=1.6,line_dash="solid",annotation_text=f"TRI {label} • BODY 50",annotation_position="top right")
        fig.add_hline(y=levels["upper"],line_color=colors[1],line_width=1.2,line_dash="dot",annotation_text=f"TRI {label} • UPPER 50",annotation_position="top right")
        fig.add_hline(y=levels["lower"],line_color=colors[2],line_width=1.2,line_dash="dot",annotation_text=f"TRI {label} • LOWER 50",annotation_position="bottom right")
    step=view.Time.iloc[-1]-view.Time.iloc[-2] if len(view)>1 else pd.Timedelta(minutes=5)
    fig.update_xaxes(range=[view.Time.iloc[0],view.Time.iloc[-1]+step*future],rangeslider_visible=False,showgrid=True,gridcolor="#172230",showspikes=True,spikemode="across",spikethickness=1,showline=False)
    fig.update_yaxes(side="right",showgrid=True,gridcolor="#172230",fixedrange=False,showspikes=True,spikemode="across")
    fig.update_layout(height=650,margin=dict(l=4,r=4,t=8,b=8),paper_bgcolor="#080d14",plot_bgcolor="#080d14",font=dict(color="#cbd5e1"),hovermode="x unified",dragmode="pan",legend=dict(orientation="h",y=1.02,x=0),uirevision=f"{symbol}-{tf}")
    return fig
...
def tri_control_panel():
    st.markdown('<div class="panel"><div class="section-title">🎨 TRI Line Control Center</div><div class="section-sub">Har timeframe ki TRI lines independently ON/OFF karo aur BODY / UPPER / LOWER colors set karo. Chart mein sirf selected timeframes appear honge.</div>',unsafe_allow_html=True)
    settings={}
    cols=st.columns(4)
    defaults={"1M":True,"1W":True,"1D":True,"4H":False,"1H":False,"30M":False,"15M":False,"5M":False}
    for i,label in enumerate(TRI_TFS):
        with cols[i%4]:
            enabled=st.checkbox(f"TRI {label}",value=defaults[label],key=f"tri_on_{label}")
            c1,c2,c3=st.columns(3)
            with c1: body=st.color_picker("Body", "#8e98ff", key=f"tri_body_{label}", label_visibility="collapsed")
            with c2: upper=st.color_picker("Upper", "#42dda0", key=f"tri_upper_{label}", label_visibility="collapsed")
            with c3: lower=st.color_picker("Lower", "#ff7184", key=f"tri_lower_{label}", label_visibility="collapsed")
...
# These controls are outside the live fragment: changing a control is immediate and does not wait for the timer.
...
def live_engine():
    ...
    df,source,cstat,chost=candles(symbol,TFS[tf],650)
    bids,asks,bsrc,bstat,bhost=orderbook(symbol)
    f=features(df,bids,asks)
    pred,prob,mlstat,feature_count=ml_predict(f)
    signal,confidence,combined,rs,rw,rscore,mlscore=final_state(f,pred,prob)
    ...
    with tabs[0]:
        ...
        with right:
            ...
            st.write(f"Data processing: `{elapsed:.0f} ms`")
            st.write(f"Last update: `{datetime.now().strftime('%H:%M:%S')}`")
            st.markdown('</div>',unsafe_allow_html=True)

    with tabs[1]:
        st.markdown('<div class="panel"><b>TRADINGVIEW-STYLE MARKET CHART</b><div class="section-sub">Mouse wheel = zoom • drag = pan • double click = reset • future space is reserved on the right.</div>',unsafe_allow_html=True)
        fig=make_chart(df,symbol,tf,future,tri_settings)
        st.plotly_chart(fig,use_container_width=True,config={"scrollZoom":True,"displaylogo":False,"responsive":True,"modeBarButtonsToAdd":["drawline","drawrect","eraseshape"],"doubleClick":"reset"},key="main_market_chart")
        st.markdown('</div>',unsafe_allow_html=True)
    ...
    with tabs[2]:
        if len(bids) and len(asks):
            ...
            l,r=st.columns(2)
            with l: st.dataframe(pd.DataFrame(bids[:20],columns=["Bid Price","Bid Qty"]),use_container_width=True,hide_index=True)
            with r: st.dataframe(pd.DataFrame(asks[:20],columns=["Ask Price","Ask Qty"]),use_container_width=True,hide_index=True)
    ...
    with tabs[5]:
        ...
        if not t.empty and "result" in t.columns:
            rr=t.result.astype(str).str.upper(); wins=int((rr=="WIN").sum()); losses=int((rr=="LOSS").sum()); total=wins+losses; wr=wins/total*100 if total else 0
