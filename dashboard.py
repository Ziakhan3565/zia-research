...
show_tri = st.sidebar.toggle("Show TRI Lines", value=True)
tri_enabled = {}
for _tf in ["YEARLY", "MONTHLY", "WEEKLY", "DAILY", "4H", "1H", "30M", "15M"]:
    tri_enabled[_tf] = st.sidebar.checkbox(_tf, value=True, key=f"tri_{_tf}")
...
def fetch_tri_candle(symbol, interval):
    try:
        ...
        res = requests.get(url, params={"symbol": symbol, "interval": interval, "limit": 3}, timeout=5)
        data = res.json()
...
def fetch_tri_yearly_candle(symbol):
    try:
        url = "https://data-api.binance.vision/api/v3/klines"
        res = requests.get(url, params={"symbol": symbol, "interval": "1M", "limit": 36}, timeout=5)
        data = res.json()
        ...
        monthly = pd.DataFrame(rows).set_index("Time")
        yearly = monthly.resample("YS").agg({"Open":"first", "High":"max", "Low":"min", "Close":"last"}).dropna()
        if len(yearly) < 2:
            return None
        r = yearly.iloc[-2]
...
def get_all_tri_levels(symbol):
    levels = {}
    yearly = calculate_tri_levels(fetch_tri_yearly_candle(symbol))
    if yearly:
        levels["YEARLY"] = yearly
    for name, interval in TRI_INTERVALS.items():
        tri = calculate_tri_levels(fetch_tri_candle(symbol, interval))
        if tri:
            levels[name] = tri
    return levels


def add_tri_lines(fig, tri_levels, enabled):
    for tf, tri in tri_levels.items():
        if not enabled.get(tf, True):
            continue
        color = TRI_COLORS[tf]
        fig.add_hline(y=tri["body_50"], line_color=color, line_width=2.5, line_dash="solid", opacity=0.90, layer="above")
        fig.add_hline(y=tri["upper_50"], line_color=color, line_width=1, line_dash="dot", opacity=0.55, layer="above")
        fig.add_hline(y=tri["lower_50"], line_color=color, line_width=1, line_dash="dot", opacity=0.55, layer="above")
    return fig
...
if not df.empty and len(df) >= 3 and len(bids) > 0 and len(asks) > 0:
    ...
    with col_chart:
        ...
        if direction == "LONG":
            ...
            forecast_prices = np.repeat(close_p, forecast_horizon)
    ...
    with col_risk_panel:
        ...
        ask_vol_sum = float(np.sum(asks[:, 1])) if len(asks) > 0 else 0.0
        obi_val = (bid_vol_sum - ask_vol_sum) / (bid_vol_sum + ask_vol_sum + 1e-12)
        spread_val = abs(float(asks[0, 0]) - float(bids[0, 0])) if len(bids) > 0 and len(asks) > 0 else 0.0
        spread_mid = (float(asks[0, 0]) + float(bids[0, 0])) / 2.0 if len(bids) > 0 and len(asks) > 0 else 0.0
        spread_pct = (spread_val / spread_mid * 100.0) if spread_mid else 0.0

        if obi_val >= 0.50:
            risk_status, risk_color = "LOW", "#00e676"
        elif obi_val >= 0.15:
            risk_status, risk_color = "LOW-MEDIUM", "#00e676"
        elif obi_val <= -0.50:
            risk_status, risk_color = "HIGH", "#ff5252"
        elif obi_val <= -0.15:
            risk_status, risk_color = "MEDIUM", "#ffa500"
        else:
            risk_status, risk_color = "NEUTRAL", "#38bdf8"
        ...
        st.markdown(
            ...
            <div class="metric-card" style="padding:16px; margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    ...
                    <b style="color:#00e676;">{bid_vol_sum:,.2f}</b>
                </div>
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span>Ask Volume</span>
                    <b style="color:#ff5252;">{ask_vol_sum:,.2f}</b>
                </div>
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span>Order Book Imbalance (OBI)</span>
                    <b style="color:#38bdf8;">{obi_val:+.3f}</b>
                </div>
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span>Spread</span>
                    <b>${spread_val:.2f}</b>
                ...
                <div style="display:flex;justify-content:space-between;">
                    ...
                    <b style="color:{risk_color};">{risk_status}</b>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        def calc_obi(depth):
            b = float(np.sum(bids[:depth, 1])) if len(bids) >= depth else float(np.sum(bids[:, 1]))
            a = float(np.sum(asks[:depth, 1])) if len(asks) >= depth else float(np.sum(asks[:, 1]))
        ...
        fig_obi = go.Figure(
            go.Bar(
                x=["Top 5", "Top 10", "Top 20"],
                y=[top5_obi, top10_obi, top20_obi],
                marker_color="#38bdf8",
                text=[f"{v:+.3f}" for v in [top5_obi, top10_obi, top20_obi]],
                textposition="outside",
                cliponaxis=False,
            )
        )
        fig_obi.add_hline(y=0, line_color="#64748b", line_width=1)
